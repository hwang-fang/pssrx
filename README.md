...
9. フライトごとにカルマンフィルタ（協調旋回モデル: SSR の ENU の位置・
   速度・旋回率）で平滑化する。後ろ 5 走査まで見た固定遅延平滑化で、応答 3 件の
   点の方位誤差（p90 2.3°）が応答 14 件の観測値と同程度（0.6°）になり、
   旋回中も角を切らない。速度と旋回率も出す。判定は変えず、`unconfirmed` の
   点は素通し
