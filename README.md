...
5. 位置を解く。機体は「SSR を焦点の一つとする双基地距離の回転楕円体」
   ...
   与えると地上距離 ρ の 2 次方程式になり閉形式で解ける。z は地球の曲率で ρ に
   依存するので、ρ を解いてから `geodesy` で厳密な z を求め直す反復を 2〜3 回
   行う。偽の根は検算で除き、機体が基線上に近い特異な幾何は解かない。観測量の
   分散を線形伝播した ENU の共分散も出す。大気屈折は無視し、気圧高度は標準大気
