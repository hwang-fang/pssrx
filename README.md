...
`ResolveState`（フライトの存在区間と同じ機体の組）、`SmoothState`（フライト
ごとのフィルタ）で、件数は呼び出し側の `Stats` に足す。
...
4. 幽霊を落とす。実データには 1 機が同じ走査に複数方位で現れる幽霊が実プロットと
   ...
   5 µs 以内を直接照射の候補とし、それより τ の大きいものを落とす。候補が
   複数あれば方位差 10° 以内は同じドウェルの断片として応答数最多を残し、
   10° 超は SSR 近傍の反射体経由の像として両方通す（手順 8 で決める）
...
6. 便に繋ぐ。同じスコークで、前の点からの移動が速度上限 350 m/s × Δt +
   ...
   `status: unconfirmed` として出力に残す。像（反射で連続して見える
...
7. 便の断片をフライトに連結する。便は 1 機体の飛行の断片（ガーブルで割れた
   ...
   連結できれば採用する
8. 同じ機体のフライトの重複を解消する。スコーク・高度・同時刻の τ が一致し
   方位が 10° 超離れた 2 本のフライトは同じ機体（像と実機）。像の存在区間は
   実機の存在区間に含まれるので、相手の区間からはみ出す方を実位置、
   はみ出さない方を `status: echo` とする。決められなければ両方
   `ambiguous`。反射の無い局では何もしない
9. フライトごとにカルマンフィルタ（等速 + 白色加速度、SSR の ENU の位置と
   速度）で平滑化する。後ろ 5 走査まで見た固定遅延平滑化で、応答 3 件の点の
   方位誤差（p90 2.3°）が応答 14 件の観測値と同程度（0.6°）になる。速度も出す。
   判定は変えず、`unconfirmed` の点は素通し
10. `Sink` へ書く。いまは CSV（時刻 JST、SSR、局、スコーク、気圧高度 [ft]、
    ...
    平滑化した緯度・経度・標高とその σ、ENU の速度 [m/s]）
...
集約について検討し採らなかった最適化は [PSSR.md](PSSR.md) の 14 節を参照。
