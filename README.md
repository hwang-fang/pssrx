...
処理できる範囲を取り出してから `Pair` → `Suppress` → `Locate` → `Track` → `Sink`
の関数の直列に流す。`pipeline` がブロックごとに順に呼ぶが、投入の刻みは手続きと
独立で、実時間化で 1 秒刻みになっても構造は同じ。持ち越す記録は待ち行列、
`RunState`（開いている列）、`SuppressState`（判定待ちのプロット）、
`TrackState`（開いている便と判定待ちの点）で、件数は呼び出し側の `Stats` に
足す。
...
7. `Sink` へ書く。いまは CSV（時刻 JST、SSR、局、スコーク、気圧高度 [ft]、
...
- `pssr.Synchronizer` は保持幅の上限（`MaxRetentionNs`、60 s）を超えた古い
...
集約について検討し採らなかった最適化は [PSSR.md](PSSR.md) の 11 節を参照。
...
| 質問予定の無い区間の応答 | `Synchronizer` が最大 `max_retention_ns`（60 s）保持したのち捨てる。`Pair` では直前の質問から `TauMax` 超で `Unpaired`。偽の位置は出ない |
