...
| `max_retention_ns` | 300000000000 | 質問予定と応答を溜めておく時間幅の上限 [ns]（5 分）。片方が止まっても他方が溜まり続けないための安全弁 |
...
| `sigma_azimuth_rad` | 0.001745 | ビーム中心の方位の標準偏差 [rad]（0.1 度） |
| `sigma_altitude_m` | 8.8 | 高さの標準偏差 [m]（Mode C の 100 ft 量子化 = 30.48 / √12） |

`sigma_*` は出力の σ_E / σ_N / σ_U（位置の標準偏差）にだけ効き、位置そのものは
変えない。
