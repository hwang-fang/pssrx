...
| `max_retention_ns` | 60000000000 | 質問予定と応答を溜めておく時間幅の上限 [ns]（60 s）。片方が止まっても他方が溜まり続けないための安全弁。取り出しの後に適用するので、投入の刻みより短くてもよい |
...
| `sigma_azimuth_rad` | 0.006109 | ビーム中心の方位の標準偏差 [rad]（0.35 度。実データの便のばらつきから較正） |
