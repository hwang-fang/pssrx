...
func TestBufferIsBounded(t *testing.T) {
	...
	for blk := range 30 {
		...
		if _, err := Pair(&st, &stats, params, DefaultConfig(), slog.Default(), replies, intg, upTo, false); err != nil {
			t.Fatal(err)
		}
		maxLen = max(maxLen, len(st.intg))
		if len(st.pending) > 100 {
			t.Fatalf("ブロック %d で保留が %d 件", blk, len(st.pending))
		}
	}
