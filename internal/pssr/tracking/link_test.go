...
func linkAll(t *testing.T, cfg tracking.Config, fixes []tracking.Fix, chunk int) ([]tracking.Fix, tracking.Stats) {
	t.Helper()
	if err := tracking.Validate(testParams, cfg); err != nil {
		t.Fatal(err)
	}
	var st tracking.LinkState
	...
	for i := 0; i < len(fixes); i += chunk {
		...
		out = append(out, tracking.Link(&st, &stats, testParams, cfg, fixes[i:end], end == len(fixes))...)
	}
	return out, stats
}

// tracked は fixAt に便 ID と判定を付ける。
func tracked(f tracking.Fix, track int64, status tracking.FixStatus) tracking.Fix {
...
func TestLinkJoinsFragmentsAcrossGap(t *testing.T) {
	...
	for k := 0; k <= 4; k++ {
		fixes = append(fixes, tracked(fixAt(float64(k), 0o1234, 10000, 0, 800*float64(k)), 1, tracking.FixOK))
	}
	for k := 9; k <= 11; k++ {
		fixes = append(fixes, tracked(fixAt(float64(k), 0o1234, 10000, 0, 800*float64(k)), 2, tracking.FixOK))
	}
...
func TestLinkRescuesShortFragment(t *testing.T) {
	...
	fixes = append(fixes,
		tracked(fixAt(9, 0o1234, 10000, 0, 7200), 2, tracking.FixUnconfirmed),
...
func TestLinkRefuses(t *testing.T) {
	...
	head := func() []tracking.Fix {
		var v []tracking.Fix
		for k := 0; k <= 4; k++ {
			v = append(v, tracked(fixAt(float64(k), 0o1234, 10000, 0, 800*float64(k)), 1, tracking.FixOK))
		}
		return v
	}
...
func TestLinkIsChunkInvariant(t *testing.T) {
	...
	for _, chunk := range []int{1, 2, 5} {
		got, gs := linkAll(t, tracking.DefaultConfig(), fixes, chunk)
