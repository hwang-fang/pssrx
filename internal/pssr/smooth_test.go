...
func smoothAll(t *testing.T, geom pssr.Geometry, cfg pssr.Config, fixes []pssr.Fix, chunk int) ([]pssr.Fix, pssr.Stats) {
	t.Helper()
	if err := pssr.Validate(testParams, cfg); err != nil {
		t.Fatal(err)
	}
	var st pssr.SmoothState
	...
	for i := 0; i < len(fixes); i += chunk {
		...
		out = append(out, pssr.Smooth(&st, &stats, geom, testParams, cfg, fixes[i:end], end == len(fixes))...)
	}
	return out, stats
}
...
func noisyLine(n int, flight int64, ve, vn, sigmaE, sigmaN float64, seed int64) ([]pssr.Fix, [][2]float64) {
...
func TestSmoothRecoversVelocityAndReducesError(t *testing.T) {
	...
	out, s := smoothAll(t, l.geom, pssr.DefaultConfig(), fixes, 0)
...
func TestSmoothPassesUnconfirmedAndKeepsOrder(t *testing.T) {
	...
	stray.Track, stray.Flight, stray.Status = 9, 9, pssr.FixUnconfirmed
	all := append(append([]pssr.Fix{}, fixes[:6]...), stray)
	all = append(all, fixes[6:]...)
	out, s := smoothAll(t, l.geom, pssr.DefaultConfig(), all, 4)
	if len(out) != 13 {
	...
	for i, f := range out {
		...
		if (f.Status == pssr.FixUnconfirmed) != (f.Smoothed == nil) {
...
func TestSmoothIsChunkInvariant(t *testing.T) {
	...
	whole, ws := smoothAll(t, l.geom, pssr.DefaultConfig(), fixes, 0)
	for _, chunk := range []int{1, 3, 11} {
		got, gs := smoothAll(t, l.geom, pssr.DefaultConfig(), fixes, chunk)
...
func TestSmoothBridgesGap(t *testing.T) {
	...
	out, _ := smoothAll(t, l.geom, pssr.DefaultConfig(), withGap, 0)
