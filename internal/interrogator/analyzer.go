...
// 次の呼び出しで来るので、isLast が false の間は blockEnd の手前
// putOffPeriod 以内で終わるセグメントを次のブロックへ繰り越し、境界を
// またぐドウェルの分断を避ける。ブロックの幅は問わない（1 分でも 1 秒でも
// 結果は同じ）。isLast は最後の呼び出しで、繰り越さずに全部処理する。
// 実時間では停止のときだけ真になる。
//
