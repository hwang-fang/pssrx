...
func TestPSSRParams(t *testing.T) {
	...
	p, err := pipeline.PSSRParams(ssr, st, pssr.DefaultConfig())
	if err != nil {
		t.Fatal(err)
	}
	const c = 0.299792458
	if want := 3000 + int64(math.Ceil(1275.0539493951226/c)); p.TauMinNs != want {
		t.Errorf("TauMin = %d, 期待 %d", p.TauMinNs, want)
	}
	if want := 3000 + int64(math.Ceil((2*400000+1275.0539493951226)/c)); p.TauMaxNs != want {
		t.Errorf("TauMax = %d, 期待 %d", p.TauMaxNs, want)
	}
	if p.TauMaxNs >= 2_949_900 {
		t.Errorf("TauMax %d が PRI 以上", p.TauMaxNs)
	}
	...
	if _, err := pipeline.PSSRParams(ssr, st, pssr.DefaultConfig()); err == nil {
